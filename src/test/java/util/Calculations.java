...
public class Calculations {
    ...
    static DateOperations dateOperations = new DateOperations();
    static LocalDate Date = dateOperations.getCurrentDate();

    public static JsonNode convertJSValuesToJson(String value) {
        ...
        try {
            ...
            System.out.println(" operation - API response: " + Response);
    ...
    public static void installmentValidations() {

        int installmentsCount = Response.get("data").size();
        for (int i = 0; i < installmentsCount; i++) {

            assertEquals(Response.get("data").get(i).get("principal_amount").asDouble(), Response.get("data").get(i).get("principal_paid").asDouble());
            assertEquals(Response.get("data").get(i).get("profit_amount").asDouble(), Response.get("data").get(i).get("profit_paid").asDouble());
            assertEquals(Response.get("data").get(i).get("installment_amount").asDouble(), Response.get("data").get(i).get("installment_paid").asDouble());
            assertEquals(0, Response.get("data").get(i).get("remaining_principal").asDouble());
            assertEquals(0, Response.get("data").get(i).get("remaining_profit").asDouble());
            assertEquals(0, Response.get("data").get(i).get("installment_balance").asDouble());
            assertEquals(0, Response.get("data").get(i).get("total_installments_balance").asDouble());
            assertEquals(0, Response.get("data").get(i).get("installments_principal_balance").asDouble());
            assertEquals(0, Response.get("data").get(i).get("installments_profit_balance").asDouble());
            assertEquals("fully paid", Response.get("data").get(i).get("status").asText());


        }

        System.out.println("All the installment details are updated as expected");
...
//        wallet details
        PrincipleAmount = WalletDetails.get("principle_amount").asDouble();
        ProfitAmount = WalletDetails.get("profit_amount").asDouble();
        PrinciplePaid = WalletDetails.get("principle_paid").asDouble();
        ProfitPaid = WalletDetails.get("profit_paid").asDouble();
        TotalFeesAmount = WalletDetails.get("total_fees_amount").asDouble();
        TotalFeesPaid = WalletDetails.get("total_fees_paid").asDouble();
        TotalRemainingBalance = WalletDetails.get("total_remaining_balance").asDouble();
        TotalInstallmentBalance = WalletDetails.get("total_installments_balance").asDouble();
        TotalRepaidAmount = WalletDetails.get("total_repaid_amount").asDouble();
        TotalReceivableAmount = WalletDetails.get("total_receivable_amount").asDouble();
...
//        String closing_date = FacilityDetails.get("closing_date").asText();
        ...
        long tenorInMonths = tenorInDays/7;  // -----> check if the data type needs to be int or long
...
//       Monthly Mandatory Payment Percentage
            expected_monthly_mandatory_payment_percentage = 100/tenorInMonths;
...
//            assertEquals(expected_monthly_mandatory_payment_percentage, monthly_mandatory_payment_percentage,"Issue present in monthly_mandatory_payment_percentage");
            ...
            assertEquals(expected_past_due_payment, past_due_payment,"Issue present in past_due_payment");
            assertEquals(expected_due_installment_payment, due_installment_payment,"Issue present in due_installment_payment");
